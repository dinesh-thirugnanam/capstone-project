...
const AddOfficeModal = ({ visible, office, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (isEditing && office) {
      ...
      setFormData({
        name: office.name || '',
        centerLatitude: (coordinates[1] || office.centerLatitude || '').toString(),
        centerLongitude: (coordinates[0] || office.centerLongitude || '').toString(),
        radius: (office.radius || GEOFENCE_CONFIG.DEFAULT_RADIUS).toString(),
        officeInfo: {
          address: office.officeInfo?.address || '',
          workingHours: {
            start: office.officeInfo?.workingHours?.start || '09:00',
            end: office.officeInfo?.workingHours?.end || '17:00',
          },
          workingDays: office.officeInfo?.workingDays || ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
        },
      });
    } else {
  ...
  const validateForm = () => {
    ...
    } else if (radius < GEOFENCE_CONFIG.MIN_RADIUS || radius > GEOFENCE_CONFIG.MAX_RADIUS) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to save office location. Please try again.');
    } finally {
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(value) => handleInputChange('officeInfo.address', value)}
                multiline
                numberOfLines={2}
              />
            </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              <View style={styles.halfInput}>
                ...
                <TextInput
                  style={[
                    styles.input,
                    formErrors.centerLatitude && styles.inputError
                  ]}
                  placeholder="e.g., 40.7128"
                  value={formData.centerLatitude}
                  onChangeText={(value) => handleInputChange('centerLatitude', value)}
                  keyboardType="numeric"
                />
                {formErrors.centerLatitude && (
                  <Text style={styles.errorText}>{formErrors.centerLatitude}</Text>
                )}
              </View>
              ...
              <View style={styles.halfInput}>
                ...
                <TextInput
                  style={[
                    styles.input,
                    formErrors.centerLongitude && styles.inputError
                  ]}
                  placeholder="e.g., -74.0060"
                  value={formData.centerLongitude}
                  onChangeText={(value) => handleInputChange('centerLongitude', value)}
                  keyboardType="numeric"
                />
                {formErrors.centerLongitude && (
                  <Text style={styles.errorText}>{formErrors.centerLongitude}</Text>
                )}
              </View>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              />
              {formErrors.radius && (
                <Text style={styles.errorText}>{formErrors.radius}</Text>
              )}
              <Text style={styles.helperText}>
                Geofence radius for automatic attendance tracking
              </Text>
            </View>
          </View>
            ...
            <View style={styles.row}>
              <View style={styles.halfInput}>
                ...
                <TextInput
                  style={[
                    styles.input,
                    formErrors.startTime && styles.inputError
                  ]}
                  placeholder="09:00"
                  value={formData.officeInfo.workingHours.start}
                  onChangeText={(value) => handleInputChange('workingHours.start', value)}
                />
                {formErrors.startTime && (
                  <Text style={styles.errorText}>{formErrors.startTime}</Text>
                )}
              </View>
              ...
              <View style={styles.halfInput}>
                ...
                <TextInput
                  ...
                  onChangeText={(value) => handleInputChange('workingHours.end', value)}
                />
                {formErrors.endTime && (
                  <Text style={styles.errorText}>{formErrors.endTime}</Text>
                )}
              </View>
            </View>

            <Text style={styles.helperText}>
              Use 24-hour format (HH:MM). Working days are Monday-Friday by default.
            </Text>
          </View>
