...
const AdminDashboard = ({ navigation }) => {
  ...
  const handleAddOffice = async (officeData) => {
    try {
      ...
      Alert.alert('Error', error.message || 'Failed to create office location');
    }
  };
  ...
  const handleUpdateOffice = async (officeData) => {
    try {
      const response = await ApiService.updateGeofence(selectedOffice._id || selectedOffice.id, officeData);
      ...
      Alert.alert('Error', error.message || 'Failed to update office location');
    }
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await ApiService.deleteGeofence(officeId);
              
              if (response.success) {
                await loadOffices(); // Refresh list
                Alert.alert('Success', 'Office location deleted successfully');
              } else {
                throw new Error(response.message || 'Failed to delete office');
              }
            } catch (error) {
              console.error('Failed to delete office:', error);
              Alert.alert('Error', error.message || 'Failed to delete office location');
            }
          },
        },
      ]
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateText}>
        Create your first office location to start tracking employee attendance
      </Text>
      <TouchableOpacity 
        style={styles.emptyStateButton}
        onPress={() => setShowAddModal(true)}
      >
        <Text style={styles.emptyStateButtonText}>Add First Office</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        ...
        <View style={styles.statItem}>
          <Text style={styles.statValue}>
            {offices.filter(office => office.isActive !== false).length}
          </Text>
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={loadOffices} />
        }
